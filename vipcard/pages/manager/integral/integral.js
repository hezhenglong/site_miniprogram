...
Page({
  ...
  data: {
    radioItems: [
      { name: '增加积分（用于奖励和升级）', value: '1', checked: true },
      { name: '消费积分（用于活动支付）', value: '0' }
    ],
    cardNum: '',
    name: '',
    phone: '',
  ...
  checkBalance: function () {
    //检查积分是否充足
    if (parseInt(this.data.price) > parseInt(this.data.integral)) {
      wx.showToast({
        title: '积分不足',
        icon: 'none',
        duration: 2000
  ...
  submit: function () {
    ...
    wx.request({
      method: 'POST',
      url: 'https://e20.xin/api/wx/integral/option',
      data: {
